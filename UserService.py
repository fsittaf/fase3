...
from User import SystemUser, User, Admin
...
class UserService:
    ...
    def get(self, id) -> SystemUser:
        return self.repository.get(id)

    def get_by_email(self, email) -> SystemUser:
        return self.repository.get_by_email(email)

    def get_all(self) -> list:
        return self.repository.get_all()

    def add(self, user: SystemUser, session_user) -> None:
        self.validator.validate(user, session_user)
        self.repository.add(user)

    # Validar permissão
    def update(self, old_user: SystemUser, session_user) -> None:
        self.repository.update(old_user, session_user)
    ...
    def get_by_name(self, name):
        return next(
            (user for user in self.repository.get_all() if user.name == name), None
        )
